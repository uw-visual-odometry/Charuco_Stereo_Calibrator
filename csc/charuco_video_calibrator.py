...
class CharucoStereoCalibrator(CharucoCalibrator):
    ...
    def process_images(self, images_left, images_right):
        ...
        print("Good frames: ", frame_idx, "/", frame_count)
    ...
    def stereo_calibration(self, camera_matrix_L, dist_L, camera_matrix_R, dist_R):
        ...
        for i in range(min(len(self.idL), len(self.idR))):
            ...
            if len(common_ids) > 0:
                ...
                if len(indices_left) == len(indices_right):  # Only append if counts match
    ...
    def save_matrices(self, camera_matrix_L, dist_L, camera_matrix_R, dist_R):
        ...
        cv_file = cv.FileStorage("stereoMap_charuco.xml", cv.FILE_STORAGE_WRITE)
    ...
    def process_video(self, video_left, video_right):
        """Process stereo images to find Charuco corners."""

        # TODO: Capture videos
        left_cap = cv.VideoCapture(video_left)
        right_cap = cv.VideoCapture(video_right)
        ...
        #indexing the frames
        frame_idx = -1
        frame_count = int(min(left_cap.get(cv.CAP_PROP_FRAME_COUNT), right_cap.get(cv.CAP_PROP_FRAME_COUNT)))
        frame_comp = 0
        # Loop until video is done go through each frame
        while left_cap.isOpened() and right_cap.isOpened():
            retL, img_left = left_cap.read()
            retR, img_right = right_cap.read()
            ...
            if gray_left.shape != (
                ...
                raise ValueError(
                    f"File size and frame size do not match. file: {frame_idx}"
                )
            ...
            if charuco_corners_L is None or charuco_corners_R is None:
                log_message(
                    f"Charuco board couldn't be detected. Image pair for frame: {frame_idx}",
                    level="ERROR",
                )
                continue
            ...
            if self.debug:
                ...
                left_debug_path = os.path.join(
                    debug_dir, f"charuco_left_{frame_idx}.png"
                )
                right_debug_path = os.path.join(
                    debug_dir, f"charuco_right_{frame_idx}.png"
                )
                cv.imwrite(left_debug_path, img_left)
                cv.imwrite(right_debug_path, img_right)
        ...
        left_cap.release()
        right_cap.release()
        print("Total good frames: ", frame_comp, "/", frame_count)

    def perform_video_calibration(self, video_left, video_right):
        """Main function to perform stereo calibration."""
        log_message("Starting stereo image processing...", "INFO")
        self.process_video(video_left, video_right)
    ...
    def save_rectified_video(self, video_left, video_right):
        """Save both fully rectified images and ROI-cropped images."""
        ...
        frame_width = int(left_cap.get(cv.CAP_PROP_FRAME_WIDTH))
        frame_height = int(left_cap.get(cv.CAP_PROP_FRAME_HEIGHT))
        
        
        fourcc = cv.VideoWriter_fourcc(*'mp4v')
        video_dir = "output/csc/rectified/video"
        os.makedirs(video_dir, exist_ok=True)
        outL = cv.VideoWriter('output/csc/rectified/video/full_l.mp4', fourcc, fps, (frame_width, frame_height))
        outR = cv.VideoWriter('output/csc/rectified/video/full_r.mp4', fourcc, fps, (frame_width, frame_height))
        outC = cv.VideoWriter('output/csc/rectified/video/full_c.mp4', fourcc, fps, (frame_width*2, frame_height))
        ...
        while left_cap.isOpened() and right_cap.isOpened():
            retL, imgL = left_cap.read()
            retR, imgR = right_cap.read()
            if not retL or not retR:
                break
            ...
            combined_image = cv.hconcat([rectifiedL, rectifiedR])

            outL.write(rectifiedL)
            outR.write(rectifiedR)
            outC.write(combined_image)
...
if __name__ == "__main__":
    ...
    frame_size_w = 1440 #4608 // 2

    # if below is None, then algorithm figure this out.
    f_in_mm = 6 #4.74
    pixel_size_mm = 3.45e-3 * 2 #1.4e-3 * 2  # binning
    ...
    left_show = "../images/AC_split/l/left_half12.jpg"
    right_show = "../images/AC_split/r/right_half12.jpg"
    ...
    # stereo_calibrator.perform_video_calibration(video_left, video_right)  
    # video_left = "./Data/Collection3/straight_data_closer_left.mp4"
